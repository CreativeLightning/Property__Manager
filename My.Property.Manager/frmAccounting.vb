...
Public Class frmAccounting
    ...
    Private Sub PrintReport(sender As Object, e As PrintPageEventArgs)
        ...
        If currentPropertyIndex < properties.Rows.Count Then
            ...
            e.Graphics.DrawString($"PROPERTY ID: {propertyID}, {streetNumber} {streetName}, {aptSuiteNumber}, {city}, {state} {zip}", New Font("Arial", 12, FontStyle.Bold), Brushes.Black, New PointF(100, yPos))
            yPos += 30

            ' Print payments
            Dim totalPayments As Decimal = 0
            If propertyPayments IsNot Nothing AndAlso propertyPayments.Any() Then
                e.Graphics.DrawString("PAYMENTS:", New Font("Arial", 12, FontStyle.Bold), Brushes.Black, New PointF(100, yPos))
                yPos += 30
                For Each payment In propertyPayments
                    e.Graphics.DrawString($"DATE: {payment.Field(Of Date)("PaymentDate")}, AMOUNT: {payment.Field(Of Decimal)("Amount"):C}, TYPE: {payment.Field(Of String)("PaymentType")}, NOTES: {payment.Field(Of String)("Notes")}", New Font("Arial", 12), Brushes.Black, New PointF(100, yPos))
                    totalPayments += payment.Field(Of Decimal)("Amount")
                    yPos += 30
                Next
                e.Graphics.DrawString($"TOTAL PAYMENTS: {totalPayments:C}", New Font("Arial", 12, FontStyle.Bold), Brushes.Black, New PointF(100, yPos))
                yPos += 30
            Else
                e.Graphics.DrawString("NO PAYMENTS AVAILABLE.", New Font("Arial", 12, FontStyle.Italic), Brushes.Black, New PointF(100, yPos))
                yPos += 30
            End If
            ...
            If propertyCharges IsNot Nothing AndAlso propertyCharges.Any() Then
                e.Graphics.DrawString("EXPENSES:", New Font("Arial", 12, FontStyle.Bold), Brushes.Black, New PointF(100, yPos))
                yPos += 30
                For Each charge In propertyCharges
                    e.Graphics.DrawString($"DATE: {charge.Field(Of Date)("DateBilled")}, TOTAL: {charge.Field(Of Decimal)("Total"):C}, NOTES: {charge.Field(Of String)("ChargeNotes")}", New Font("Arial", 12), Brushes.Black, New PointF(100, yPos))
                    totalCharges += charge.Field(Of Decimal)("Total")
                    yPos += 30
                Next
                e.Graphics.DrawString($"TOTAL EXPENSES: {totalCharges:C}", New Font("Arial", 12, FontStyle.Bold), Brushes.Black, New PointF(100, yPos))
                yPos += 30
            Else
                e.Graphics.DrawString("NO EXPENSES AVAILABLE.", New Font("Arial", 12, FontStyle.Italic), Brushes.Black, New PointF(100, yPos))
                yPos += 30
            End If

            ' Calculate and print net income
            Dim netIncome As Decimal = totalPayments - totalCharges
            e.Graphics.DrawString($"NET INCOME FOR THIS PROPERTY: {netIncome:C}", New Font("Arial", 12, FontStyle.Bold), Brushes.Black, New PointF(100, yPos))
            yPos += 30
        ...
        ElseIf currentPropertyIndex = properties.Rows.Count Then
            ...
            e.Graphics.DrawString("FINAL SUMMARY", New Font("Arial", 14, FontStyle.Bold), Brushes.Black, New PointF(100, 150))
            yPos = 200
            e.Graphics.DrawString($"TOTAL PAYMENTS: {totalPaymentsAllProperties:C}", New Font("Arial", 12, FontStyle.Bold), Brushes.Black, New PointF(100, yPos))
            yPos += 30
            e.Graphics.DrawString($"TOTAL EXPENSES: {totalChargesAllProperties:C}", New Font("Arial", 12, FontStyle.Bold), Brushes.Black, New PointF(100, yPos))
            yPos += 30
            Dim totalNetIncome As Decimal = totalPaymentsAllProperties - totalChargesAllProperties
            e.Graphics.DrawString($"TOTAL NET INCOME: {totalNetIncome:C}", New Font("Arial", 12, FontStyle.Bold), Brushes.Black, New PointF(100, yPos))
            yPos += 30
